...
/* Returns a pointer to the record so the caller can edit fields directly */
...
/* Linear search, case-insensitive partial match */
 ...
 * TODO: could use qsort here, but keeping manual implementation
...
void db_sort_by_name_asc(StudentDB *db) {
    ...
    for (int i = 0; i < db->count - 1; i++) {
        ...
        for (int j = i + 1; j < db->count; j++) {
            if (strcasecmp(db->data[j]->name, db->data[minIdx]->name) < 0) {
                minIdx = j;
            }
        }
        if (minIdx != i) {
...
int db_save(const StudentDB *db, const char *filename) {
    ...
    /* Write magic number and record count */
...
int db_load(StudentDB *db, const char *filename) {
    ...
    if (fread(&count, sizeof(count), 1, fp) != 1 || count < 0) {
        fprintf(stderr, "Error: corrupt record count in file.\n");
        fclose(fp);
        return -1;
    }
    ...
    for (int i = 0; i < count; i++) {
        ...
        /* Re-compute GPA in case file has inconsistency */
        s->gpa = student_calc_gpa(s);
        if (db_add(db, s) != 0) {
            student_free(s);   /* duplicate or alloc fail */
        }
    }
...
void report_top_n(const StudentDB *db, int n) {
    ...
    /* Make a temporary sorted copy of pointers (bubble sort) */
    ...
    for (int i = 0; i < db->count - 1; i++) {
        for (int j = 0; j < db->count - 1 - i; j++) {
            if (tmp[j]->gpa < tmp[j + 1]->gpa) {
                Student *swap = tmp[j]; tmp[j] = tmp[j+1]; tmp[j+1] = swap;
            }
        }
    }
...
void report_best_per_course(const StudentDB *db) {
    ...
    /* Collect unique course names and their best GPA */
    char   courses[64][MAX_COURSE_LEN];
    float  best_gpa[64];
    int    best_idx[64];    /* index into db->data */
    int    num_courses = 0;
...
/* Uses a temporary sorted array to find median without modifying db */
void report_median_gpa(StudentDB *db) {
    ...
    /* Simple sort for the temporary array */
    for (int i = 0; i < db->count - 1; i++) {
        for (int j = 0; j < db->count - 1 - i; j++) {
            if (gpas[j] > gpas[j + 1]) {
...
int input_unique_id(const StudentDB *db, const char *prompt) {
    while (1) {
        ...
        printf("  ID %d is already taken. Please choose another.\n", id);
    }
}
